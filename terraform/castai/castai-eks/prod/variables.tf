...
variable "vpc_id" {
  ...
  default = {
    "us-east-1" = "vpc-0e297549887e3bfa4"
    "us-west-2" = "vpc-089c76fb40a216e11"
  }
}
...
variable "castai_workload_scaling_policies" {
  ...
  default = {
    ...
    AA_Stability = {
      ...
      cpu    = { overhead = 0.15, min = 0.10, look_back_period_seconds = 604800 }
      memory = { overhead = 0.20, max = 10240.0, look_back_period_seconds = 604800 }
    }
  ...
  type = map(object({
    ...
    constraints = optional(object({
      ...
      custom_priority = optional(object({
        instance_families = optional(list(string), ["r6a", "r6i", "r5", "r5a", "r5b", "r7i", "r7a", "r8a", "m6i", "m6a", "c6i", "c6a", "m5", "m5a", "c5", "c5a", "m7i", "m7i-flex", "m7a", "m8a", "c7i", "c7i-flex", "c7a", "c8a"])
        spot              = optional(bool, false)
        on_demand         = optional(bool, true)
      }), {})
    }), {})
...
variable "castai_rebalancing_schedule" {
  ...
  type = map(object({
    enabled                  = optional(bool, true)
    schedule_cron            = optional(string, "CRON_TZ=America/Chicago 0 21 */1 * *")
    savings_percentage       = optional(number, 10)
